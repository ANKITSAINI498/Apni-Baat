const categories={
 greeting:/\b(hello|hi|hey|heyya|good morning|good night|gm|gn|kaise ho|kya haal)\b/i,
 ...
 coding:/\b(java|react|spring|boot|code|coding|bug|api|javascript|css|html|program)\b/i,
...
export function detectCategory(text){for(const [key,re] of Object.entries(categories)){if(re.test(text))return key}return 'general'}
export function generateReply(text,responses){const key=detectCategory(text);const pool=responses[key]||responses.general;return pool[Math.floor(Math.random()*pool.length)]}
